...
import { AlertCircleIcon, Trash2Icon, UploadCloudIcon, XIcon } from "lucide-react";
...
export interface SingleImageDropzoneProps extends React.HTMLAttributes<HTMLInputElement> {
   ...
   */
  dropzoneOptions?: Omit<DropzoneOptions, "disabled" | "onDrop" | "maxFiles" | "multiple">;
}
        ...
        >
          ...
          {displayUrl ? (
            ...
            >
              ...
              <div className="font-medium">drag & drop an image or click to select</div>
              {maxSize && <div className="text-xs">Max size: {formatFileSize(maxSize)}</div>}
            </div>
          )}
          ...
          {displayUrl && !disabled && fileState && fileState.status !== "COMPLETE" && (
            <button
              type="button"
