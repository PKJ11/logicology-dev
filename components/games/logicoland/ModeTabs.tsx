...
export default function ModeTabs({ currentMode, onModeChange }: ModeTabsProps) {
  ...
  return (
    <div className="flex flex-wrap justify-center gap-2 p-2 md:gap-4">
      {modes.map((mode) => (
        <motion.button
          key={mode.id}
          ...
          className={`font-outfit rounded-2xl px-6 py-3 text-lg font-semibold transition-all duration-200 ${
            currentMode === mode.id
