...
export default function FeedbackToast({ feedback, onClose }: FeedbackToastProps) {
  ...
  const getBgColor = () => {
    switch (feedback.type) {
      ...
      case "error":
        return "bg-gradient-to-r from-red-100 to-pink-100 border-red-300";
      case "warning":
        return "bg-gradient-to-r from-yellow-100 to-amber-100 border-yellow-300";
      case "info":
        return "bg-gradient-to-r from-blue-100 to-sky-100 border-blue-300";
      default:
        return "bg-gradient-to-r from-blue-100 to-sky-100 border-blue-300";
    }
  };
  ...
  return (
    <div className="fixed bottom-4 left-1/2 z-50 -translate-x-1/2 transform animate-fade-in">
      <div className={`${getBgColor()} flex items-center gap-3 rounded-2xl border-2 p-4 shadow-lg`}>
