        ...
        <div className="grid gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3">
          {products.map((product) => {
            ...
            return (
              <article
                key={product.id}
                className="group relative overflow-hidden rounded-4xl border border-black/5 bg-white shadow-soft transition-shadow focus-within:ring-2 focus-within:ring-brand-teal/30 hover:shadow-brand"
              >
                {/* Image */}
                <div className="p-5">
                ...
                <div className="px-6 pb-6">
                  <h2 className="line-clamp-2 font-heading text-xl font-semibold text-gray-900">
                    {product.title}
                  </h2>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="font-heading text-lg font-bold text-brand-tealDark">
                      {price} <span className="text-base text-gray-500">{currency}</span>
                    </div>
          ...
          {(!products || products.length === 0) && (
            <div className="col-span-full">
              <div className="rounded-4xl border border-dashed border-brand-teal/30 bg-white p-10 text-center shadow-soft">
                ...
                <p className="mt-2 text-gray-600">Add products in Shopify and refresh this page.</p>
              </div>
            </div>
          )}
        </div>
