...
import { DigitSumGameProps, GameStats } from "@/app/voice-games/lib/types";
import { generateDigitSumQuestion, calculateScore, getStreakMessage } from "@/app/voice-games/lib/gameLogic";
...
export const DigitSumGame: React.FC<DigitSumGameProps> = ({ difficulty, gameMode }) => {
  const [question, setQuestion] = useState(() => generateDigitSumQuestion(difficulty));
  ...
  const nextQuestion = useCallback(() => {
    setQuestion(generateDigitSumQuestion(difficulty));
    setLastResult(null);
    setShowAnswer(null);
  ...
  // Advance to next question once phase returns to idle after a result
  ...
  // Ask the current question whenever it changes and we're idle
  useEffect(() => {
    if (phase === "idle") {
      askQuestion(
    ...
    return (
      <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
        <p className="text-red-500 text-xl">
          ⚠️ Speech recognition is not supported in your browser. Try Chrome or Edge.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-md p-4 flex justify-around text-center">
        <div><p className="text-2xl font-bold text-blue-600">{stats.score}</p><p className="text-xs text-gray-500 uppercase tracking-wide">Score</p></div>
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 text-center space-y-8">
        ...
        <div className="bg-blue-50 rounded-xl p-6">
          <p className="text-lg text-gray-500 mb-2">What is the sum of</p>
          <p className="text-5xl font-black text-blue-700 tracking-tight">
            {question.question}
          </p>
        </div>

        {/* Mic button — phase drives all colour, no flicker */}
        <VoiceMicButton
          phase={phase}
