...
export default function SubscriptionModal({
  ...
  const [currentSubscription, setCurrentSubscription] = useState<CurrentSubscriptionData | null>(
    null
  );
    ...
    try {
      ...
      if (
        ...
        const currentTierData = tiers.find(
          (t) => t.id === currentSubscription.subscription?.tierId
        );
      ...
      if (
        ...
        if (Number(selectedTier) === 1) {
          ...
          setError(
            "Downgrades to other paid plans are only allowed at the end of your current billing cycle"
          );
          setIsProcessing(false);
          return;
        }
  ...
  const getUpgradeInfo = (tierId: number) => {
    ...
    const upgradeAmount = Math.round(selectedTierData.price - currentTierData.price);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative z-10 max-h-[90vh] w-full max-w-[90vw] overflow-y-auto rounded-2xl bg-white shadow-2xl sm:max-w-[80vw] md:max-w-[70vw] lg:max-w-[60vw] xl:max-w-[60vw]">
        <div className="p-6 md:p-8">
          {/* Header */}
          <div className="mb-6 flex items-center justify-between md:mb-8">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 md:text-3xl">Choose Your Plan</h2>
              {currentSubscription?.currentMonth && (
                <p className="mt-1 text-sm text-gray-500">
                  Current billing period:{" "}
                  {new Date(currentSubscription.currentMonth.startDate).toLocaleDateString()} -{" "}
                  {new Date(currentSubscription.currentMonth.endDate).toLocaleDateString()}
                </p>
              )}
            </div>
                ...
                <div>
                  <p className="text-sm text-blue-800">
                    ...
                    <span className="font-bold">
                      {tiers.find((t) => t.id === currentSubscription.subscription?.tierId)?.name}
                    </span>{" "}
                    plan
                  </p>
            ...
            {tiers.map((tier) => {
              ...
              return (
                <div
                  ...
                  className={`relative rounded-xl border-2 p-4 transition-all hover:shadow-xl md:rounded-2xl md:p-6 ${
                    selectedTier === tier.id
                      ? `border-orange-500 bg-gradient-to-br ${tier.color} text-white`
                      : isSelectable
                        ? "cursor-pointer border-gray-200 hover:border-orange-200"
                        : "cursor-not-allowed border-gray-200 opacity-60"
                  }`}
                >
                  {/* Lock overlay for non-selectable tiers */}
                  {!isSelectable && (
                    <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-gray-100/50 backdrop-blur-[1px]">
                      <span className="rounded-full bg-gray-800 px-3 py-1 text-xs text-white shadow-lg">
                        🔒 Month end only
                      </span>
                    </div>
                  )}
                  ...
                  <div className="mb-3 md:mb-4">
                    ...
                    <div className="mt-1 md:mt-2">
                      <span className="text-2xl font-bold md:text-3xl">₹{tier.price}</span>
                      {tier.price > 0 && (
                        <span className="text-xs opacity-80 md:text-sm">/month</span>
                      )}
                    </div>
                    <p
                      className={`mt-1 text-xs md:mt-2 md:text-sm ${selectedTier === tier.id ? "text-white/80" : "text-gray-500"}`}
                    >
                      {tier.description}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 md:space-y-3">
                    {tier.features.slice(0, 4).map((feature, index) => (
                      <div key={index} className="flex items-start space-x-2">
                        ...
                        <span
                          className={`text-xs md:text-sm ${selectedTier === tier.id ? "text-white" : "text-gray-600"}`}
                        >
                          {feature}
                        </span>
                      </div>
                    ...
                    {tier.features.length > 4 && (
                      <div
                        className={`text-xs opacity-75 ${selectedTier === tier.id ? "text-white" : "text-gray-500"}`}
                      >
                  ...
                  {isCurrentTier && (
                    <div className="mt-3 md:mt-4">
                      <span className="inline-block rounded-full bg-green-100 px-2 py-1 text-xs font-semibold text-green-800 md:px-3 md:py-1">
                        Current Plan
                      </span>
                    </div>
                  )}
                  ...
                  {upgradeInfo && selectedTier === tier.id && (
                    <div className="mt-3 rounded-lg bg-white/20 p-2 text-xs">
                  ...
                  {!isSelectable && selectMessage && (
                    <div className="mt-3 rounded-lg bg-gray-100 p-2 text-xs text-gray-600">
          ...
          <div className="flex flex-col-reverse justify-end gap-3 sm:flex-row sm:space-x-4">
            <button
              onClick={onClose}
              className="w-full rounded-lg border border-gray-300 px-4 py-2 font-medium text-gray-700 transition-colors hover:bg-gray-50 sm:w-auto md:px-6 md:py-3"
            >
              Cancel
            </button>
            <button
              ...
              className="w-full rounded-lg bg-orange-500 px-6 py-2 font-medium text-white shadow-lg shadow-orange-200 transition-all hover:bg-orange-600 hover:shadow-xl disabled:cursor-not-allowed disabled:bg-gray-400 disabled:shadow-none sm:w-auto md:px-8 md:py-3"
            >
              {isProcessing ? (
