...
export async function GET(req: NextRequest) {
  try {
    ...
    const newSubscriptions = await db.collection("subscriptions").countDocuments({
      status: "active",
    ...
    // Get content by type
    const worksheets = await db.collection("content").countDocuments({ type: "worksheet" });
    const wordwall = await db.collection("content").countDocuments({ type: "wordwall" });
    const mindstamp = await db.collection("content").countDocuments({ type: "mindstamp" });
    const games = await db.collection("content").countDocuments({ type: "game" });
    const assessments = await db.collection("content").countDocuments({ type: "assessment" });
    const externalLinks = await db.collection("content").countDocuments({ type: "external_link" });
    ...
    // Get tier details
    const tiers = await db.collection("tiers").find({}).sort({ id: 1 }).toArray();
      ...
      recentActivities.map(async (activity: any) => {
        ...
        if (activity.adminId) {
          const admin = await db
            ...
            .findOne({ _id: new ObjectId(activity.adminId) }, { projection: { name: 1 } });
