  ...
  try {
    ...
    if (existingSubscription) {
      ...
      if (!currentTier || !newTier) {
        return NextResponse.json({ success: false, error: "Invalid tier IDs" }, { status: 400 });
      }
      ...
      if (Math.abs(amount - expectedUpgradeAmount) > 1) {
      ...
      await subscriptions.updateOne({ _id: existingSubscription._id }, { $set: subscription });
    } else {
