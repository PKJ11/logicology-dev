...
export default function MasculineFeminineGame() {
  ...
  useEffect(() => {
    ...
    const distractors = shuffle(
      pairs.filter(p => p.id !== pair.id).map(p => p.feminine)
    ).slice(0, 15);
  ...
  useEffect(() => {
    ...
    timerIntervalRef.current = setInterval(() => {
      setTimer(t => { if (t <= 1) { setDone(true); return 0; } return t - 1; });
    }, 1000);
  ...
  useEffect(() => {
    ...
    const tick = (ts: number) => {
      ...
      setDecorComets(prev => prev.map(c => {
  ...
  const handleAnswer = useCallback((word: string, idx: number) => {
    ...
    setTimeout(() => {
      if (isCorrect) {
        ...
        setLives(l => { const nl = l - 1; if (nl <= 0) setDone(true); return nl; });
        setFeedback(null);
      }
    }, 700);
  ...
  return (
    <div ref={containerRef} style={{ position:"fixed", inset:0, fontFamily:"'Orbitron','Exo 2',sans-serif", overflow:"hidden", background:"#0a0015" }}>
      <style>{`
      ...
      {/* BG */}
      {phase === "game" && (
        <img src="https://ik.imagekit.io/pratik2002/background.png" alt=""
        ...
        <div style={{ position:"absolute",inset:0,zIndex:50,
          ...
          {Array.from({length:80}).map((_,i)=>(
            <div key={i} style={{ position:"absolute",left:`${Math.random()*100}%`,top:`${Math.random()*100}%`,
              width:Math.random()>0.8?"3px":"1.5px",height:Math.random()>0.8?"3px":"1.5px",
              borderRadius:"50%",background:"#fff",opacity:Math.random()*0.7+0.2 }} />
          ))}

          {/* Two-column layout */}
          <div className="modal-box" style={{
            display:"flex",width:"100%",maxWidth:"1000px",
            margin:"auto",gap:"0",borderRadius:"24px",overflow:"hidden",
            ...
            {/* LEFT — Game info */}
            <div style={{ flex:1,padding:"44px 40px",display:"flex",flexDirection:"column",justifyContent:"center",borderRight:"1px solid rgba(120,80,255,0.25)" }}>
              <div style={{ fontSize:"11px",letterSpacing:"4px",color:"#a78bfa",textTransform:"uppercase",marginBottom:"10px",fontFamily:"'Exo 2',sans-serif" }}>Grade 2 · English</div>
              <h1 style={{ fontSize:"clamp(24px,3.5vw,38px)",fontWeight:900,color:"#fff",lineHeight:1.15,marginBottom:"6px",textShadow:"0 0 30px rgba(180,140,255,0.8)" }}>Find the Match</h1>
              <h2 style={{ fontSize:"clamp(15px,2vw,22px)",fontWeight:600,color:"#c4b5fd",marginBottom:"24px",fontFamily:"'Exo 2',sans-serif" }}>Masculine &amp; Feminine</h2>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"28px" }}>
                {[
                  ...
                  <div key={c.label} style={{ background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"12px",padding:"12px 14px",display:"flex",alignItems:"center",gap:"10px" }}>
                    <span style={{ fontSize:"22px" }}>{c.icon}</span>
                    <div>
                      <div style={{ color:"#fff",fontFamily:"'Exo 2',sans-serif",fontWeight:800,fontSize:"15px" }}>{c.label}</div>
                      <div style={{ color:"#9b8cc0",fontFamily:"'Exo 2',sans-serif",fontSize:"11px" }}>{c.sub}</div>
                    </div>
                  </div>
                ))}
              </div>

              <p style={{ fontSize:"13px",color:"#7a6d99",lineHeight:1.6,fontFamily:"'Exo 2',sans-serif",marginBottom:"28px" }}>
                A big comet flies across with a masculine word.<br/>
                Tap its matching feminine answer from the grid!
              </p>

              <button onClick={()=>setPhase("video")} style={{
                display:"block",width:"100%",padding:"16px",
                background:"linear-gradient(135deg,#7c3aed,#2563eb)",
                border:"none",borderRadius:"12px",color:"#fff",
                fontSize:"18px",fontWeight:800,cursor:"pointer",
                letterSpacing:"2px",boxShadow:"0 4px 28px rgba(124,58,237,0.55)",
                fontFamily:"'Exo 2',sans-serif",marginBottom:"10px",
              }}>▶ &nbsp;START GAME</button>
              <button onClick={goFullscreen} style={{
                display:"block",width:"100%",padding:"11px",
                background:"transparent",border:"1.5px solid rgba(120,80,255,0.45)",
                borderRadius:"12px",color:"#a78bfa",fontSize:"14px",
                cursor:"pointer",letterSpacing:"2px",fontFamily:"'Exo 2',sans-serif",
              }}>⛶ &nbsp;FULLSCREEN</button>
            </div>

            {/* RIGHT — Leaderboard */}
            <div style={{ width:"320px",padding:"44px 32px",display:"flex",flexDirection:"column" }}>
              <div style={{ display:"flex",alignItems:"center",gap:"10px",marginBottom:"20px" }}>
                <span style={{ fontSize:"22px" }}>🏆</span>
                <div>
                  <div style={{ color:"#fff",fontWeight:800,fontSize:"18px",fontFamily:"'Exo 2',sans-serif" }}>Leaderboard</div>
                  <div style={{ color:"#7a6d99",fontSize:"11px",fontFamily:"'Exo 2',sans-serif" }}>Top players this week</div>
                </div>
              </div>
              ...
              {LEADERBOARD.map(row=>(
                <div key={row.rank} className={`lb-row${row.rank<=3?" top3":""}`}>
                  <div style={{ width:"24px",textAlign:"center",fontSize:"16px",flexShrink:0 }}>
                    {row.medal || <span style={{ color:"#6b5d8a",fontFamily:"'Exo 2',sans-serif",fontWeight:700,fontSize:"14px" }}>#{row.rank}</span>}
                  </div>
                  <div style={{ flex:1,color:"#e0d8ff",fontFamily:"'Exo 2',sans-serif",fontWeight:700,fontSize:"14px" }}>{row.name}</div>
                  <div style={{ color:"#ffd700",fontFamily:"'Exo 2',sans-serif",fontWeight:800,fontSize:"15px" }}>{row.score}</div>
                </div>
              ))}

              <div style={{ marginTop:"auto",paddingTop:"24px",borderTop:"1px solid rgba(255,255,255,0.08)" }}>
                <div style={{ color:"#7a6d99",fontFamily:"'Exo 2',sans-serif",fontSize:"11px",textAlign:"center",lineHeight:1.6 }}>
                  Complete all 16 pairs to<br/>claim your spot on the board!
                </div>
              </div>
            </div>
      ...
      {phase === "video" && (
        <div style={{ position:"absolute",inset:0,zIndex:40,background:"#000" }}>
          <video src="https://ik.imagekit.io/pratik2002/first_video.mp4" style={{ width:"100%",height:"100%",objectFit:"cover" }} autoPlay onEnded={()=>setPhase("game")} />
          <button onClick={()=>setPhase("game")} style={{ position:"absolute",bottom:"28px",right:"28px",background:"rgba(255,255,255,0.15)",border:"1px solid rgba(255,255,255,0.3)",color:"#fff",borderRadius:"8px",padding:"8px 20px",cursor:"pointer",fontFamily:"'Exo 2',sans-serif",fontSize:"13px" }}>Skip →</button>
        </div>
      )}
      ...
      {phase === "game" && !done && currentPair && (
        <>
          {/* HUD */}
          <div style={{ position:"absolute",top:0,left:0,right:0,zIndex:30,padding:"10px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(to bottom,rgba(0,0,0,0.72) 0%,transparent 100%)" }}>
            <div style={{ color:"#fff",fontSize:"clamp(18px,2.5vw,28px)",fontWeight:700,letterSpacing:"2px" }}>{formatTime(timer)}</div>
            <div style={{ color:"#e0d0ff",fontFamily:"'Exo 2',sans-serif",fontSize:"clamp(13px,1.8vw,18px)",fontWeight:700 }}>
              Tap the matching tile
            </div>
            <div style={{ display:"flex",alignItems:"center",gap:"10px" }}>
              <div style={{ fontSize:"clamp(18px,2.5vw,26px)" }}>
                {Array.from({length:3}).map((_,i)=>(
                  <span key={i} className={`heart${i>=lives?" lost":""}`}>{i<lives?"❤️":"🖤"}</span>
                ))}
              </div>
              <span style={{ color:"#ffd700",fontSize:"clamp(14px,2vw,20px)",fontFamily:"'Exo 2',sans-serif",fontWeight:800 }}>✓ {score}</span>
            </div>
          </div>

          {/* Decorative comets — upper zone only, no text */}
          {decorComets.map(c=>(
            <div key={c.id} style={{ position:"absolute",left:`${c.x}vw`,top:`${c.y}vh`,width:c.size,height:c.size,transform:"translate(-50%,-50%)",zIndex:2,pointerEvents:"none",opacity:0.75 }}>
              <img src={COMET_IMGS[c.imgIdx]} alt="" style={{ width:"100%",height:"100%",objectFit:"contain" }}
                onError={e=>{(e.currentTarget as HTMLImageElement).style.display="none";}} />
            </div>
          ))}
              ...
              {options.map((word,idx)=>{
                ...
                return (
                  <button key={key}
                    ...
                    disabled={!!feedback}
                  >
                    <div className="btn-face">{idx+1}. {word}</div>
                  </button>
                );
              })}
            </div>
      ...
      {/* ════════════ DONE ════════════ */}
      {phase==="game"&&done&&(
        <div style={{ position:"absolute",inset:0,zIndex:60,background:"radial-gradient(ellipse at 50% 40%,#120030 0%,#03000a 100%)",display:"flex",alignItems:"center",justifyContent:"center" }}>
          <div className="done-box" style={{ background:"linear-gradient(135deg,rgba(30,0,80,0.97),rgba(0,10,60,0.97))",border:"2px solid rgba(120,80,255,0.6)",borderRadius:"24px",padding:"52px 64px",textAlign:"center",maxWidth:"480px",width:"90%",boxShadow:"0 0 80px rgba(120,80,255,0.5)" }}>
            <div style={{ fontSize:"64px",marginBottom:"16px" }}>{lives>0?"🎉":"💫"}</div>
            <h2 style={{ color:"#fff",fontSize:"32px",fontWeight:900,marginBottom:"8px",textShadow:"0 0 20px rgba(180,140,255,0.8)" }}>{lives>0?"Great Job!":"Game Over"}</h2>
            <p style={{ color:"#a78bfa",fontSize:"20px",fontFamily:"'Exo 2',sans-serif",marginBottom:"32px" }}>Score: <strong style={{ color:"#ffd700" }}>{score}</strong></p>
            <button onClick={()=>{setPhase("modal");setDone(false);}} style={{ padding:"14px 40px",background:"linear-gradient(135deg,#7c3aed,#2563eb)",border:"none",borderRadius:"12px",color:"#fff",fontSize:"16px",fontWeight:700,cursor:"pointer",letterSpacing:"2px",boxShadow:"0 4px 24px rgba(124,58,237,0.5)" }}>↩ Play Again</button>
          </div>
        </div>
      )}
    </div>
