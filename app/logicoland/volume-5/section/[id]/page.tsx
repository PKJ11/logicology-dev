...
export default function SectionPage() {
  ...
  const totalPuzzles =
    sectionId <= 2
      ? sectionId === 1
        ? colourCrawlPuzzles.length
        : arrowsAddressPuzzles.length
      : 1;
  ...
  const handleStepModeClick = (row: number, col: number) => {
    ...
    const isAdjacent = Math.abs(row - lastPosition[0]) + Math.abs(col - lastPosition[1]) === 1;
  ...
  const handleRouteClick = (routeId: string) => {
    ...
    route.moves.forEach((move, index) => {
      setTimeout(() => {
        ...
        if (
          nextPos[0] >= 0 &&
          nextPos[0] < puzzle.grid.length &&
          nextPos[1] >= 0 &&
          nextPos[1] < puzzle.grid[0].length
        ) {
        ...
        if (index === route.moves.length - 1) {
          const reachedTarget =
            currentPos[0] === puzzle.target[0] && currentPos[1] === puzzle.target[1];
  ...
  const getSectionName = () => {
    switch (sectionId) {
      case 1:
        return "Colour Crawl";
      case 2:
        return "Arrows Address";
      case 3:
        return "Right Route";
      case 4:
        return "Knowing Knight";
      default:
        return "Unknown";
    }
  };
  ...
  const renderGameModeSelector = () => {
    ...
    return (
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="rounded-2xl bg-gradient-to-r from-blue-50 to-sky-50 p-4 shadow-soft">
          <h3 className="mb-3 text-center text-xl font-bold text-gray-800">Game Mode</h3>
  ...
  const renderPuzzleContent = () => {
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {puzzle.routes.map((route) => {
              ...
              return (
                ...
                >
                  <h4 className="mb-4 text-xl font-bold">{route.name}</h4>
                  ...
                  {isSelected && isAnimating && (
                    <div className="mb-4 animate-pulse font-medium text-blue-600">
                      Animating path...
                    </div>
                  )}
                  ...
                  {isSelected && !isAnimating && (
                    <div
                      className={`mb-4 font-bold ${correct ? "text-green-600" : "text-red-600"}`}
                    >
          ...
          {isAnimating && selectedRoute && (
            <div className="mt-8">
              ...
              <p className="mt-2 text-gray-600">
                ...
                {puzzle.routes.find((r) => r.id === selectedRoute)?.moves.length || 0} moves
              </p>
            </div>
          )}
        </div>
      ...
      return (
        <div className="text-center">
          ...
          <p className="mb-6 text-gray-600">
            Remember: Knights move in an L-shape (2 squares in one direction, then 1 square
            perpendicular)
          </p>
    ...
    return (
      <>
        ...
        <div className="mb-8 rounded-2xl bg-gradient-to-r from-orange-100 to-amber-100 p-6">
          ...
          {gameMode === "step-mode" && (
            <div className="mt-4 text-center">
              <div className="inline-block rounded-full bg-blue-100 px-4 py-2 font-bold text-blue-800">
  ...
  return (
    <Volume5Layout sectionNumber={sectionId} sectionName={getSectionName()}>
      <TopBar
      ...
      <div className="mb-8">{renderPuzzleContent()}</div>
      ...
      <div className="mb-8 flex flex-wrap justify-center gap-4">
        {gameState === "playing" ? (
          <>
            <SecondaryButton icon={HelpCircle} onClick={handleHint} size="lg">
              Show Hint
            </SecondaryButton>

            <SecondaryButton icon={RefreshCw} onClick={resetGameState} size="lg">
              Reset
            </SecondaryButton>

            <SecondaryButton icon={SkipForward} onClick={handleNextPuzzle} size="lg" variant="soft">
              Skip Puzzle
            </SecondaryButton>
          </>
        ) : gameState === "completed" ? (
          <PrimaryButton icon={Check} onClick={handleNextPuzzle} size="lg">
      ...
      <div className="rounded-2xl bg-gray-50 p-6">
        <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
          ...
          <div className="flex gap-4">
            <div className="rounded-full bg-orange-100 px-3 py-1 font-bold text-orange-800">
              Difficulty: {getDifficulty()}
            </div>
            <div className="rounded-full bg-blue-100 px-3 py-1 font-bold text-blue-800">
