        ...
        <View style={styles.section}>
          ...
          {gradedResults.map((result, idx) => (
            <View key={idx} style={{ marginBottom: 10 }}>
              <Text style={styles.question}>
                Q{result.question.id}: {result.question.text}
              </Text>
...
const GuestNameModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex animate-fade-in items-center justify-center bg-[#020B39]/80 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md rounded-[2rem] border border-white/10 bg-[#16256F] p-8 shadow-card">
        <button
          onClick={onClose}
          className="absolute right-4 top-4 text-[#636D8C] transition-colors hover:text-[#E2E3E8]"
        ...
        <h2 className="font-display mb-2 text-2xl text-[#E2E3E8] md:text-3xl">Welcome, Guest</h2>
        <p className="mb-6 text-[#636D8C]">Please enter your name to continue</p>

        <form onSubmit={handleSubmit}>
          <div className="mb-6">
            <label htmlFor="guest-name" className="mb-2 block text-sm font-medium text-[#E2E3E8]">
              Your Name
            </label>
            <input
              type="text"
              ...
              className={`w-full rounded-xl border bg-[#1A2F88] px-4 py-3 text-[#E2E3E8] placeholder-[#636D8C] transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#2A72F7]/50 ${error ? "border-[#E0505D]" : "border-white/10"} `}
              placeholder="Enter your full name"
              autoFocus
            />
            {error && <p className="mt-2 text-sm text-[#E0505D]">{error}</p>}
          </div>
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 rounded-xl border border-white/10 px-6 py-3 text-[#E2E3E8] transition-colors hover:bg-white/5"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="flex-1 rounded-xl bg-[#2A72F7] px-6 py-3 font-medium text-white transition-colors hover:bg-[#2A72F7]/90"
            >
              Continue
            </button>
          </div>
...
const ProgressBar = ({ progress }: { progress: number }) => (
  <div className="h-2 w-full overflow-hidden rounded-full bg-[#1A2F88]">
    <div
...
const QuestionCard = ({
  ...
  return (
    <div className="animate-fade-in rounded-[2rem] border border-white/5 bg-[#16256F]/30 p-6 shadow-card backdrop-blur-sm md:p-8">
      <h2 className="font-display mb-8 text-2xl text-[#E2E3E8] md:text-3xl">{question.text}</h2>
      ...
      <div className="space-y-3">
        {question.options.map((option) => (
          <button
            ...
            className={`w-full rounded-2xl border-2 p-4 text-left transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] md:p-6 ${
              isSelected(option.id)
                ? "shadow-glow border-[#2A72F7] bg-[#2A72F7]/10"
                : "border-white/5 bg-[#1A2F88]/30 hover:bg-[#1A2F88]/50"
            } `}
          >
            <div className="flex items-start gap-4">
              <span
                className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg text-sm font-medium ${
                  isSelected(option.id)
              ...
              <span className="flex-1 text-base text-[#E2E3E8]/90 md:text-lg">{option.text}</span>

              {question.type === "multi" && isSelected(option.id) && (
                <svg
                  className="h-6 w-6 flex-shrink-0 text-[#2A72F7]"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
      ...
      {question.type === "multi" && (
        <p className="mt-4 text-sm text-[#636D8C]">Select all that apply</p>
      )}
    </div>
  );
};
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-12 text-center">
        <h1 className="font-display mb-4 text-4xl text-[#E2E3E8] md:text-5xl">
          Results for <span className="text-[#2A72F7]">{userName}</span>
        </h1>

        <div className="inline-block rounded-[2rem] border border-white/5 bg-[#16256F]/50 p-8 backdrop-blur-sm">
          <div className={`font-display mb-2 text-7xl md:text-8xl ${getScoreColor()}`}>
            {score}/{totalGraded}
          </div>
          <p className="text-xl text-[#636D8C]">{getScoreMessage()}</p>
        </div>
      </div>
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <p className="mb-3 font-medium text-[#E2E3E8]">{result.question.text}</p>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-start gap-2">
                    ...
                    <span className={result.isCorrect ? "text-[#2A72F7]" : "text-[#E0505D]"}>
                      {Array.isArray(result.userAnswer)
                        ? result.userAnswer
                            .map((id) => result.question.options.find((o) => o.id === id)?.text)
                            .join(", ")
                        : result.question.options.find((o) => o.id === result.userAnswer)?.text ||
                          "Not answered"}
                    </span>
                  ...
                  {!result.isCorrect && (
                    <div className="flex items-start gap-2">
                      <span className="min-w-[80px] text-[#636D8C]">Correct:</span>
      ...
      <div className="flex flex-col justify-center gap-4 md:flex-row">
        {isClient && (
          <PDFDownloadLinkComponent
            ...
            className="shadow-glow inline-flex items-center justify-center gap-2 rounded-xl bg-[#2A72F7] px-8 py-4 font-medium text-white transition-colors hover:bg-[#2A72F7]/90"
          >
            {({ loading }) => (
              <>
                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button
          onClick={onRestart}
          className="rounded-xl border border-white/10 px-8 py-4 text-[#E2E3E8] transition-colors hover:bg-white/5"
        >
...
export default function QuizPage() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const savedResponse = await saveQuizResponse(user._id, user.name, user.isGuest, answers);
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#020B39]">
      ...
      <div className="pointer-events-none fixed inset-0">
        ...
        />
        <div className="absolute left-1/2 top-1/2 h-[1000px] w-[1000px] -translate-x-1/2 -translate-y-1/2 transform animate-pulse-soft rounded-full bg-gradient-to-r from-[#FB6A31] via-[#E0505D] to-[#AA5D8B] opacity-10 blur-[150px] filter" />
      </div>
        ...
        <div className="container mx-auto">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden items-center gap-2 md:flex">
              <span className="text-sm text-[#636D8C]/60">ABM Mastery Assessment</span>
              <div className="mx-2 h-4 w-px bg-[#4F556F]" />
      ...
      <div className="container relative z-10 mx-auto px-4 py-8 md:py-16">
        {currentScreen === "name" && (
          <div className="mx-auto max-w-4xl text-center">
            <h1 className="font-display mb-6 bg-gradient-to-r from-[#E2E3E8] to-[#E2E3E8]/80 bg-clip-text text-4xl text-transparent md:text-5xl lg:text-6xl">
              Select Your Name
            </h1>
            <p className="mx-auto mb-12 max-w-2xl text-lg text-[#636D8C]">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 lg:grid-cols-3">
              {NAMES.map((name, index) => (
                <button
                  key={name}
                  onClick={() =>
                    name === "Guest" ? setShowGuestModal(true) : handleNameSelect(name, false)
                  }
                  className={`hover:shadow-glow animate-slide-up group relative rounded-3xl border border-white/5 bg-[#16256F]/30 p-6 shadow-soft backdrop-blur-sm transition-all duration-300 md:p-8`}
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-[#2A72F7]/0 via-[#2A72F7]/5 to-[#AA5D8B]/0 opacity-0 transition-opacity duration-500 group-hover:opacity-100" />

                  <span className="relative z-10 text-xl font-medium text-[#E2E3E8] md:text-2xl">
                    {name}
                  </span>

                  {name !== "Guest" && (
                    <div className="absolute right-3 top-3 h-2 w-2 animate-pulse-soft rounded-full bg-[#2A72F7]" />
                  )}
                </button>
              ))}
            </div>
            ...
            <div className="mt-12 text-sm text-[#636D8C]">
              <span className="inline-flex items-center gap-2">
                <span className="h-1.5 w-1.5 rounded-full bg-[#2A72F7]"></span>
                Team members
              </span>
              <span className="ml-6 inline-flex items-center gap-2">
                <span className="h-1.5 w-1.5 rounded-full bg-[#FB6A31]"></span>
                Guest access
              </span>
            </div>
          </div>
        ...
        {currentScreen === "quiz" && user && (
          <div className="mx-auto max-w-3xl">
            <div className="mb-8">
              ...
              <div className="mt-4 text-center">
                ...
                {!currentQuestion.graded && (
                  <span className="ml-3 rounded-full bg-[#AA5D8B]/20 px-2 py-1 text-xs text-[#AA5D8B]">
                    Not graded
                  </span>
                )}
              </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 mt-8 border-t border-white/5 bg-[#16256F]/80 p-4 backdrop-blur-md md:relative md:bottom-auto md:border-0 md:bg-transparent md:p-0">
              <div className="container mx-auto max-w-3xl">
                <div className="flex gap-3 md:justify-between">
                  <button
                    onClick={handlePrevious}
                    disabled={currentQuestionIndex === 0}
                    className={`flex-1 rounded-xl border px-6 py-3 transition-all duration-200 md:flex-none ${
                      currentQuestionIndex === 0
                        ? "cursor-not-allowed border-white/5 text-[#4F556F]"
                  ...
                  {currentQuestionIndex === totalQuestions - 1 ? (
                    <button
                      ...
                      disabled={isSaving}
                      className={`shadow-glow flex-1 rounded-xl bg-[#2A72F7] px-8 py-3 font-medium text-white transition-all duration-200 hover:bg-[#2A72F7]/90 disabled:cursor-not-allowed disabled:bg-[#4F556F]/30 disabled:text-[#636D8C] md:flex-none`}
                    >
                      {isSaving ? (
                    ...
                    <button
                      ...
                      className={`flex-1 rounded-xl px-8 py-3 font-medium transition-all duration-200 md:flex-none ${
                        isNextDisabled()
                          ? "cursor-not-allowed bg-[#4F556F]/30 text-[#636D8C]"
                          : "shadow-glow bg-[#2A72F7] text-white hover:bg-[#2A72F7]/90"
                      } `}
                    >
                      Next
                    </button>
        ...
        {currentScreen === "results" && user && (
          <ResultsScreen userName={user.name} answers={answers} onRestart={handleRestart} />
        )}
      </div>
