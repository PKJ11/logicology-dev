...
function AnagramGame() {
  ...
  const [animations, setAnimations] = useState<{[key: string]: 'swap' | 'bounce' | 'success' | null}>({});
  ...
  const handleCellClick = (row: number, col: number) => {
    ...
    // If this is the second selection and in same row
      ...
      if (isCorrect) {
        ...
        const correctPositions = newPuzzles[row].scrambled.split('')
        ...
        >
          ...
          <div className="mx-auto mt-4 max-w-md">
            <div className="mb-2 flex justify-between text-sm text-brand-tealDark/70">
              <span>Progress</span>
              <span>{completedPuzzles.length} / {puzzles.length} completed</span>
            </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-5 gap-3">
                      {row.map((cell, cellIndex) => {
                        ...
                        const isCorrectPosition = puzzle.scrambled[cellIndex] === puzzle.word[cellIndex];
                        ...
                        return (
                          <motion.button
                            ...
                            className={`
                              relative flex aspect-square w-full items-center justify-center rounded-xl
                              text-2xl font-bold transition-all duration-300
                              ${getCellAnimationClass(`${rowIndex}-${cellIndex}`)}
                              ${
                                isCompleted
                                  ? 'bg-gradient-to-br from-green-200 to-green-300 text-green-800 shadow-green'
                                  : isSelected
                                  ? 'bg-gradient-to-br from-blue-200 to-blue-300 text-blue-800 shadow-blue scale-110 z-10'
                                  : isCorrectPosition
                          ...
                          >
                            ...
                            {/* Position indicator */}
                            <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-black/10 text-xs font-normal">
                              {cellIndex + 1}
                            </span>
...
function WordFormationGame() {
  ...
  const [animations, setAnimations] = useState<{[key: string]: 'success' | 'error' | 'drop' | 'pulse' | null}>({});
  ...
  return (
    <section ref={sectionRef} className="w-full bg-gradient-to-b from-brand-grayBg to-blue-50 py-12 sm:py-16 md:py-20">
      <motion.div
        initial={{ opacity: 0, y: 50 }}
        animate={isInView ? { opacity: 1, y: 0 } : {}}
        transition={{ duration: 0.7 }}
          ...
          <div className="mx-auto mt-6 flex max-w-2xl flex-col items-center justify-between gap-4 sm:flex-row">
            <div className="flex-1">
              <div className="mb-1 flex justify-between text-sm text-brand-tealDark/70">
                <span>Puzzle {currentPuzzleIndex + 1} of {level2Puzzles.length}</span>
          ...
          <div className="rounded-2xl bg-gradient-to-b from-white to-blue-50 p-6 shadow-inner">
              ...
              <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                  ...
                  <div className="mt-1 text-sm text-brand-tealDark/70">
                    Target: <span className="font-bold">{currentPuzzle.targetWord.length}</span> letters
                  </div>
                </div>
            ...
            <div className="mb-8">
              ...
              <div className={`grid ${topGridCols} gap-4`}>
                {availableLetters.map((letter, index) => (
                  <div
                    key={`source-${index}`}
                    className={`flex aspect-square items-center justify-center rounded-xl text-3xl font-bold transition-all duration-300 ${
                      getAnimationClass(`source-${index}`)
                    } ${
                      letter
                        ? 'cursor-grab bg-gradient-to-br from-blue-200 to-blue-400 text-blue-900 shadow-lg hover:shadow-xl hover:scale-105 active:cursor-grabbing active:scale-95'
                        : 'bg-gradient-to-br from-gray-100 to-gray-300 text-transparent cursor-default'
                    } ${isDragging && letter ? 'scale-110 z-20' : ''}`}
                    draggable={!!letter}
            ...
            <div className="mb-8 flex items-center justify-center">
              <div className="relative flex flex-col items-center">
                <div className="mb-2 text-sm text-brand-tealDark/70 font-medium">
                  Drag letters down to empty slots ↓
                </div>
                <div className="flex items-center justify-center h-12 w-12 rounded-full bg-gradient-to-r from-blue-100 to-purple-100">
                  <svg className="h-8 w-8 text-purple-500 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className={`grid ${bottomGridCols} gap-4`}>
                  {wordCells.map((letter, index) => (
                    <div
                      key={`target-${index}`}
                      className={`flex aspect-square items-center justify-center rounded-xl text-3xl font-bold transition-all duration-300 ${
                        getAnimationClass(`target-${index}`)
                      } ${
                        letter === null
                          ? 'cursor-pointer bg-gradient-to-br from-gray-100 to-gray-300 text-gray-500 ring-4 ring-dashed ring-gray-400 hover:bg-gradient-to-br hover:from-gray-200 hover:to-gray-400 hover:ring-gray-500'
                          : 'bg-gradient-to-br from-green-200 to-green-400 text-green-900 shadow-lg'
                      } ${hoveredDropIndex === index && letter === null ? 'scale-110 ring-8 ring-green-400/50 bg-gradient-to-br from-green-100 to-green-300' : ''}`}
                      onDragOver={(e) => handleDragOver(index, e)}
                      onDragLeave={handleDragLeave}
                      onDrop={(e) => handleDrop(index, e)}
                      onClick={() => handleWordCellClick(index)}
                      title={letter === null ? "Drop letter here (click to remove)" : `Click to remove ${letter}`}
                    >
                      {letter === null ? (
                        <div className="text-center p-2 md:p-10">
                          <div className="text-xl md:text-3xl text-gray-600 font-medium ">?</div>
                          <div className=" text-gray-500 mt-1 text-xl md:text-3xl">Slot {index + 1}</div>
                        </div>
                      ) : (
              ...
              <div className="mt-4 text-center">
                <div className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-blue-50 to-indigo-50 px-4 py-2 shadow-sm">
                  ...
                  <span className="font-bold text-brand-teal text-lg">
                    {wordCells.filter(cell => cell !== null).length}/{currentPuzzle.targetWord.length}
                  </span>
                </div>
              </div>
            </div>
            ...
            {isCompleted && (
              ...
              >
                ...
                <div className="text-5xl font-bold tracking-widest text-green-900 animate-success-pop">
                ...
                <button
                  ...
                  className={`rounded-full px-8 py-3 font-medium transition-all duration-300 ${
                    wordCells.some(cell => cell === null) || isCompleted
                      ? 'cursor-not-allowed bg-gray-100 text-gray-400'
                      : 'bg-gradient-to-r from-brand-teal to-brand-coral text-white hover:scale-105 hover:shadow-xl text-lg'
                  }`}
                >
            ...
            <div className="mt-8 rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 p-4">
              <h3 className="mb-3 text-center text-lg font-bold text-brand-tealDark">How to Play:</h3>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-brand-tealDark/80">
      ...
      <style jsx global>{`
        ...
        .cursor-grab {
          cursor: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='32' height='32' viewport='0 0 32 32' style='fill:black;font-size:24px;'><text y='50%'>👆</text></svg>") 16 0, auto;
        }
