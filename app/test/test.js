class HidatoValidator {
  constructor(grid) {
    ...
    this.maxNumber =
      this.givenNumbers.length > 0 ? Math.max(...this.givenNumbers.map((g) => g.num)) : 0;
  ...
  validate() {
    ...
    if (!end)
      return {
        isValid: false,
        isUnique: false,
        message: `Puzzle must contain number ${this.totalCells}`,
      };
    ...
    if (this.solutions.length === 0) {
      ...
      return {
        isValid: true,
        isUnique: true,
        message: "Valid Hidato puzzle with a unique solution",
      ...
      return {
        isValid: true,
        isUnique: false,
        message: `Puzzle has multiple solutions (not unique)`,
      ...
      for (let j = 0; j < this.cols; j++) {
        ...
        if (v < 1 || v > N)
          return {
            isValid: false,
            isUnique: false,
            message: `Number ${v} at (${i},${j}) out of range [1..${N}]`,
          };
        if (seen.has(v))
  ...
  findSolutions(grid, row, col, currentNum) {
    ...
    for (const [nr, nc] of moves) {
      ...
      if (nextGiven && nextGiven.num - nextNum <= 12) {
        ...
        prune = !this.bfsCanReachOnGrid(
          grid,
          nr,
          nc,
          nextGiven.row,
          nextGiven.col,
          nextGiven.num - nextNum,
          nextNum
        );
      }
...
function verifySolution(original, solution) {
  ...
  // All cells filled
  for (let i = 0; i < rows; i++)
    for (let j = 0; j < cols; j++) if (!solution[i][j]) fail(`Cell (${i},${j}) is empty`);
  ...
  const pos = (num) => {
    for (let i = 0; i < rows; i++)
      for (let j = 0; j < cols; j++) if (solution[i][j] === num) return { r: i, c: j };
    return null;
  };
  for (let n = 1; n < N; n++) {
    const a = pos(n),
