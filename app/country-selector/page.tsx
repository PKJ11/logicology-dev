  ...
  return (
    <div className="flex flex-col items-center justify-center p-8">
      <div className="relative h-16 w-16">
        <div className="absolute inset-0 animate-spin rounded-full border-4 border-[#E8F0FE] border-t-[#4A90E2]"></div>
        <div className="absolute inset-2 flex items-center justify-center">
          <span className="text-2xl">🌍</span>
        </div>
      </div>
      <p className="mt-4 font-medium text-[#4A90E2]">{message}</p>
    </div>
  );
}
...
function LobbyScreen({
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#F0F7FF] to-[#FFFFFF] p-4">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="mb-1 text-4xl font-bold text-[#2C3E50]">
        ...
        {/* Simple Tab Switcher */}
        <div className="mb-6 flex rounded-xl bg-[#F5F7FA] p-1">
        ...
        {/* Clean Form */}
        <div className="space-y-5 rounded-2xl border border-[#E5E9F0] bg-white p-6 shadow-lg">
          {/* Name with avatar */}
          <div>
            <label className="mb-2 block text-sm font-medium text-[#2C3E50]">Your Name</label>
            <div className="flex gap-3">
              <button
                onClick={() => setAvatarIndex((prev) => (prev + 1) % ANIMAL_AVATARS.length)}
                className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#F5F7FA] text-2xl transition-colors hover:bg-[#E8F0FE]"
              >
              ...
              <input
                ...
                maxLength={16}
                className="flex-1 rounded-xl bg-[#F5F7FA] px-4 py-2 text-[#2C3E50] placeholder-[#BDC3C7] outline-none transition-all focus:ring-2 focus:ring-[#4A90E2]"
              />
            </div>
          </div>
          ...
          {tab === "create" ? (
            <>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-[#2C3E50]">Game Mode</label>
              ...
              <div>
                ...
                <input
                  ...
                  className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-[#E5E9F0]"
                  style={{
                    background: `linear-gradient(90deg, #4A90E2 0%, #4A90E2 ${(rounds - 3) * 8.33}%, #E5E9F0 ${(rounds - 3) * 8.33}%)`,
                  }}
                />
                <div className="mt-1 flex justify-between text-xs text-[#7F8C8D]">
                  <span>3</span>
                  <span>8</span>
                  <span>15</span>
                </div>
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-[#2C3E50]">Room Code</label>
              <input
                value={roomId}
                onChange={(e) => setRoomId(e.target.value.toUpperCase())}
                placeholder="Enter 5-letter code"
                maxLength={5}
                className="w-full rounded-xl bg-[#F5F7FA] px-4 py-3 text-center uppercase tracking-widest text-[#2C3E50] placeholder-[#BDC3C7] outline-none transition-all focus:ring-2 focus:ring-[#4A90E2]"
              />
            </div>
          )}
          ...
          {/* Simple Error */}
          {error && (
            <div className="rounded-xl bg-[#FFE5E5] px-4 py-3 text-center text-sm text-[#E74C3C]">
              ⚠️ {error}
            </div>
          )}
          ...
          <button
            ...
            className="w-full rounded-xl bg-[#4A90E2] py-3 font-medium text-white transition-colors hover:bg-[#357ABD] disabled:cursor-not-allowed disabled:opacity-40"
          >
        ...
        {/* Simple Footer */}
        <p className="mt-6 text-center text-xs text-[#BDC3C7]">
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#F0F7FF] to-[#FFFFFF] p-4">
      <div className="w-full max-w-md">
        <div className="mb-6 text-center">
          <h2 className="mb-2 text-2xl font-bold text-[#2C3E50]">Waiting Room</h2>
          <p className="text-[#7F8C8D]">Share this code with friends</p>
        </div>

        {/* Room Code */}
        <div
          className="mb-4 cursor-pointer rounded-2xl border-2 border-[#E5E9F0] bg-white p-6 text-center transition-colors hover:border-[#4A90E2]"
          onClick={() => {
        ...
        >
          <div className="mb-2 text-sm text-[#7F8C8D]">
            {copied ? "✨ Copied!" : "Click to copy"}
          </div>
          <div className="text-4xl font-bold tracking-widest text-[#2C3E50]">{room.id}</div>
        </div>

        {/* Mode Badge */}
        <div className="mb-4 flex items-center gap-3 rounded-xl border border-[#E5E9F0] bg-white px-4 py-3">
        ...
        {/* Players List */}
        <div className="mb-5 rounded-2xl border border-[#E5E9F0] bg-white p-5">
          <div className="mb-3 text-sm font-medium text-[#7F8C8D]">
            Players ({room.players.length}/4)
          </div>
          <div className="space-y-2">
            {room.players.map((p: any, index: number) => (
              <div key={p.id} className="flex items-center gap-3 rounded-xl bg-[#F5F7FA] p-2">
                <span className="text-2xl">
                  {ANIMAL_AVATARS[index % ANIMAL_AVATARS.length].emoji}
                </span>
                <span className="flex-1 text-[#2C3E50]">{p.name}</span>
                {p.id === room.host && (
                  <span className="rounded-full bg-[#4A90E2] px-2 py-1 text-xs text-white">
                    HOST
                  </span>
                )}
            ...
            {Array.from({ length: 4 - room.players.length }).map((_, i) => (
              <div
                key={i}
                className="flex items-center gap-3 rounded-xl bg-[#F5F7FA] p-2 opacity-50"
              >
                <span className="text-2xl">❓</span>
        ...
        {isHost ? (
          <button
            ...
            className="w-full rounded-xl bg-[#4A90E2] py-3 font-medium text-white transition-colors hover:bg-[#357ABD] disabled:opacity-40"
          >
            {room.players.length < 2 ? "Need 1 more player" : "Start Game"}
          </button>
        ) : (
          <div className="py-3 text-center text-[#7F8C8D]">Waiting for host to start...</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#F0F7FF] to-[#FFFFFF] p-4">
      <div className="w-full max-w-md text-center">
        ...
        <h2 className="mb-2 text-3xl font-bold text-[#2C3E50]">Game Over!</h2>
        <p className="mb-6 text-[#7F8C8D]">{reason || "Great job everyone!"}</p>

        <div className="mb-6 space-y-2 rounded-2xl border border-[#E5E9F0] bg-white p-6">
          {sorted.map((p, i) => (
            <div key={p.id} className="flex items-center gap-3 rounded-xl bg-[#F5F7FA] p-3">
              <span className="w-8 text-xl">{["🥇", "🥈", "🥉", "🎮"][i]}</span>
              <span className="flex-1 text-left text-[#2C3E50]">{p.name}</span>
...
function ActiveGame({
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-[#F0F7FF] to-[#FFFFFF]">
      {/* Simple Top Bar */}
      <div className="flex items-center justify-between border-b border-[#E5E9F0] bg-white px-4 py-3">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <div className="flex w-64 flex-col gap-4 overflow-y-auto border-r border-[#E5E9F0] bg-white p-4">
          {/* Target Country */}
          <div className="rounded-xl bg-[#F5F7FA] p-4">
            <div className="mb-1 text-xs text-[#7F8C8D]">Find this country</div>
            <div className="text-xl font-bold text-[#2C3E50]">{roundCountry?.name ?? "..."}</div>
            {showFunFact && funFact && (
              <div className="mt-2 rounded-lg bg-[#E8F0FE] p-2 text-xs text-[#4A90E2]">
                ✨ {funFact}
              </div>
            )}
          </div>
          ...
          {/* Leaderboard */}
          <div className="rounded-xl bg-[#F5F7FA] p-4">
            <div className="mb-3 text-xs text-[#7F8C8D]">Leaderboard</div>
            <div className="space-y-2">
              {sorted.map((p, i) => (
                <div key={p.id} className="flex items-center gap-2">
                  <span className="w-6 text-sm">{["🥇", "🥈", "🥉", "🎮"][i]}</span>
                  <span className="flex-1 truncate text-sm text-[#2C3E50]">{p.name}</span>
                  <span className="font-bold text-[#4A90E2]">{p.score}</span>
                </div>
              ))}
            </div>
          ...
          {/* My Score */}
          {myPlayer && (
            <div className="rounded-xl bg-[#E8F0FE] p-3 text-center">
              <div className="mb-1 text-xs text-[#7F8C8D]">Your Score</div>
        ...
        <div className="flex flex-1 flex-col p-4">
          <div className="flex-1 overflow-hidden rounded-2xl border border-[#E5E9F0] bg-white">
            <div className="h-full w-full">
              <WorldMap onCountryClick={handleClick} />
            </div>
          </div>
          <div className="mt-2 flex items-center justify-between text-xs text-[#BDC3C7]">
...
export default function CountrySelectorPage() {
  ...
  const connectionDot = (
    <div className="fixed right-3 top-3 z-50 flex items-center gap-2 rounded-full border border-[#E5E9F0] bg-white px-3 py-1 shadow-sm">
      <div
        className="h-2 w-2 rounded-full"
        style={{ background: connected ? "#7ED321" : "#E74C3C" }}
      />
      <span className="text-xs text-[#7F8C8D]">{connected ? "Connected" : "Connecting..."}</span>
    </div>
  );

  if (!connected) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#F0F7FF] to-[#FFFFFF]">
        {connectionDot}
  ...
  if (gameOver) {
    return (
      <>
        {connectionDot}
        <GameOverScreen players={gameOver.players} reason={gameOver.reason} onBack={resetGame} />
      </>
    );
  }
